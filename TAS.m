...
N = 64;                           % Number of reflecting elements in LIS
M = 2;                            % M-ary signaling order
bpcu = log2(M);                   % Bits per channel use (SE)
bits_total = bpcu * 5e4;          % Total number of bits (increase for higher SNR)
...
for snr_idx = 1:length(SNR_dB)
    ...
    for iteration = 1:(bits_total / bpcu)
        ...
        x = ss(dec);   % Virtual PSK symbol exp(j*omega_m)

        % Channel generation
        h = (randn(N, 1) + 1i * randn(N, 1)) / sqrt(2);

        % Intelligent Access Point
        Phi_intelligent = exp(-1i * angle(h));
        A_intelligent = sum(h .* Phi_intelligent);
        n = sigma * (randn + 1i * randn);
        r_intelligent = A_intelligent * x + n;
        SNR_intelligent = abs(A_intelligent)^2 / sigma^2;
        if SNR_intelligent < threshold_SNR
            outage_count_intelligent = outage_count_intelligent + 1;
        end
        [~, detected_intelligent] = min(arrayfun(@(k) norm(r_intelligent - A_intelligent * ss(k))^2, 1:M));
        decoded_intelligent(kk:kk + bpcu - 1) = de2bi(detected_intelligent - 1, bpcu, 'left-msb');

        % Blind Access Point
        % Phi_blind = ones(N, 1);
        % A_blind = sum(h .* Phi_blind);
        % r_blind = A_blind * x + n;
        % SNR_blind = abs(A_blind)^2 / sigma^2;
        % if SNR_blind < threshold_SNR
        %     outage_count_blind = outage_count_blind + 1;
        % end
        % [~, detected_blind] = min(arrayfun(@(k) norm(r_blind - A_blind * ss(k))^2, 1:M));
...
legend('Intelligent Access Point', 'Blind Access Point');
