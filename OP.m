...
% Initialize array to store outage probability values
OutageProb = zeros(size(Pt));
...
inst_SNR = bsxfun(@rdivide, Pt', No * PL_i); % Adjusted for path loss PL_i

% Define threshold SNR (adjust as needed)
SNR_th_dB = 10; % Threshold SNR in dB
SNR_th = 10^(SNR_th_dB/10); % Threshold SNR in linear scale

% Compute outage probability using SNR threshold
for k = 1:length(Pt)
    outage_count = sum(inst_SNR(k, :) < omega * SNR_th);
    OutageProb(k) = outage_count / num_nodes;
end
