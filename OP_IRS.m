...
% Function for path loss calculation (f_R(r)) for IRS scenario
n = 3;
B = [735, -1190, 455] / 72;
beta = [2, 4, 6];
D = max(distances);
...
% Compute instantaneous SNR for each Pt (IRS only)
inst_SNR_irs = Pt' ./ (No * PL_i);

% Ensure inst_SNR_irs is real and non-negative
inst_SNR_irs = max(inst_SNR_irs, eps); % Avoid inst_SNR_irs being zero

% Compute Outage Probability using Q-function approximation for Nakagami-m fading
OutageProb = zeros(size(Pt_dB));
for k = 1:length(Pt_dB)
    OutageProb(k) = mean(1 - gammainc(m, SNR_th ./ inst_SNR_irs(k)));
end

% Plot: Outage Probability vs Transmit Power (Pt_dB)
figure;
semilogy(Pt_dB, OutageProb, 'b-o', 'LineWidth', 1.5);
